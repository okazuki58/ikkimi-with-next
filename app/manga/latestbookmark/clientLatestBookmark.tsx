...
const LatestBookmarks = () => {
  ...
  const formatBookmarkDate = (bookmarkDate: string) => {
    ...
    // タイムゾーンの差を調整
    const timezoneOffset = now.getTimezoneOffset() * 60 * 1000; // ミリ秒単位
    const adjustedBookmarkTime = new Date(
      bookmarkTime.getTime() - timezoneOffset
    );

    const diffInSeconds = Math.floor(
      (now.getTime() - adjustedBookmarkTime.getTime()) / 1000
    );
